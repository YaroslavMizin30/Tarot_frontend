const TRANSLATIONS_RU = {
  // Знаки зодиака
  'Aries': 'Овен',
  'Taurus': 'Телец',
  'Gemini': 'Близнецы',
  'Cancer': 'Рак',
  'Leo': 'Лев',
  'Virgo': 'Дева',
  'Libra': 'Весы',
  'Scorpio': 'Скорпион',
  'Sagittarius': 'Стрелец',
  'Capricorn': 'Козерог',
  'Aquarius': 'Водолей',
  'Pisces': 'Рыбы',

  'is in sign': 'находится в знаке',

  'The moon is void of course. Time for rest, routine activities or wrapping old matters.':
    'Сегодня Луна без курса. Время для отдыха, рутины и завршения старых дел.',
  'A time for new beginnings and planting seeds of intention.':
    'Время новых начинаний и посадки «семян» намерений.',
  'A time to act, overcoming early obstacles':
    'Время действовать, преодолевая первые препятствия',
  'A moment to exercise willpower and overcome doubts':
    'Момент, когда нужно проявить волю и преодолеть сомнения',
  'A time for patience, analysis, and faith in your path':
    'Время терпения, анализа и веры в свой путь',
  'A powerful moment for release and celebrating achievements':
    'Время отпускания и эмоциональных откровений',
  "Energy begins to wane. Share experiences, express appreciation for what you've received":
    'Энергия начинает спадать. Делитесь опытом, выражайте признательность за полученное',
  'Let go of what no longer serves.': 'Отпустите то, что больше не нужно.',
  'A time for solitude, meditation, and preparing for new beginnings':
    'Время для уединения, медитаций и подготовки к новым начинаниям',

  'Phase': 'Фаза',
  'Next phases': 'Следующие фазы',
  'Moon day': 'Лунный день',
  'Illumination': 'Освещённость',
  'Moon age': 'Возраст Луны',
  'Position in sign': 'Положение в знаке',
  'Next phase': 'Ближайшая фаза',
  'days': 'дн.',
  'Supermoon': 'Суперлуние',
  'Micromoon': 'Микролуние',
  'Blue Moon': 'Голубая Луна',
  'Black Moon': 'Чёрная Луна',
  'Harvest Moon': 'Урожайная Луна',
  "Hunter's Moon": 'Луна охотника',
  'Lunar eclipse nearby': 'Рядом лунное затмение',
  'Plans for this day': 'Планы на этот день',
  'Telegram reminder': 'Напоминание в Telegram',
  'Loading plans...': 'Загружаем планы...',
  'Delete plan': 'Удалить план',
  'What would you like to remember?': 'Что вы хотите запланировать?',
  'Plan text': 'Текст плана',
  'Remind at': 'Напомнить в',
  'Add plan': 'Добавить',
  'Unable to save plans. Try again later.':
    'Не удалось сохранить планы. Попробуйте позже.',
  'Loading moon calendar...': 'Загружаем лунный календарь...',
  'Unable to load the moon calendar.':
    'Не удалось загрузить лунный календарь.',
  'Moon calendar data is unavailable.': 'Данные лунного календаря недоступны.',

  'Moon': 'Луна',
  'New Moon': 'Новая луна',
  'Waxing Crescent': 'Растущий серп',
  'First Quarter': 'Первая четверть',
  'Waxing Gibbous': 'Восходящая луна',
  'Full Moon': 'Полная луна',
  'Waning Gibbous': 'Убывающая луна',
  'Last Quarter': 'Последняя четверть',
  'Waning Crescent': 'Убывающий серп',

  'newMoon': 'Новолуние',
  'waxingCrescent': 'Растущий серп',
  'firstQuarter': 'Первая четверть',
  'waxingGibbous': 'Восходящая луна',
  'fullMoon': 'Полнолуние',
  'waningGibbous': 'Убывающая луна',
  'lastQuarter': 'Последняя четверть',
  'waningCrescent': 'Убывающий серп',

  'at': 'на',
};

export default TRANSLATIONS_RU;
